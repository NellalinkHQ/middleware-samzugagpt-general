...
router.post('/', async function(req, res, next) {
    try {
        ...
        const staking_plan_name = "Plan D: DEFI/WEB3 Staking";
        ...
        // Debug: Check if roi_payment_interval is defined and valid
        if (!roi_payment_interval) {
            const response = {
                status: false,
                status_code: 400,
                message: "Plan 4 ROI payment interval is not configured",
                error: {
                    missing_variable: "MODULE1_STAKING_PLAN_4_ROI_PAYMENT_INTERVAL",
        ...
        // Check if roi_payment_interval is valid
        if (!TIMESTAMP_INTERVAL_VALUES[roi_payment_interval]) {
            const response = {
                status: false,
                status_code: 400,
                message: "Invalid ROI payment interval for Plan 4",
                error: {
        ...
        // Check if MODULE1_STAKING_PLAN_4_ALLOWED_STAKING_WALLET_ID is set and not empty
        const ALLOWED_STAKING_WALLET_IDS = MODULE1_STAKING_PLAN_4_ALLOWED_STAKING_WALLET_ID.split(',');
        // Check if wallet_id is allowed
