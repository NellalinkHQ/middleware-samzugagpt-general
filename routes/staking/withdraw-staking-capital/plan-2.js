...
router.post('/:stakingTransactionID', async (req, res) => {
    try {
        ...
        const planValidation = validatePlanStaking(stakingMeta, 'plan_2');
        ...
        console.error(`${MODULE1_STAKING_PLAN_2_NAME} Capital Withdrawal Error:`, error);
        ...
        if (error.response) {
            return res.status(error.response.status).json({
                ...
                message: `${MODULE1_STAKING_PLAN_2_NAME} Capital Withdrawal Failed`,
                error: {
        ...
        return res.status(400).json({
            status: false,
            status_code: 400,
            message: `Internal server error during ${MODULE1_STAKING_PLAN_2_NAME} capital withdrawal`,
            error: {
...
// POST /staking/withdraw-staking-capital/plan-2/block-chain-external/:stakingTransactionID
router.post('/blockchain-external/:stakingTransactionID', async (req, res) => {
    try {
        ...
        console.error(`${MODULE1_STAKING_PLAN_2_NAME} Capital External Withdrawal Error:`, error);
        ...
        return res.status(400).json({
            status: false,
            status_code: 400,
            message: `Internal server error during ${MODULE1_STAKING_PLAN_2_NAME} capital external withdrawal - ` + error.message,
            error: {
                message: error.message,
                staking_transaction_id: req.params.stakingTransactionID
            }
