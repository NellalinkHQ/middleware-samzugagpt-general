...
// POST /staking/withdraw-staking-capital/plan-1/:stakingTransactionID
router.post('/:stakingTransactionID', async (req, res) => {
    try {
        ...
        // Validate that this is a Plan 1 staking
        if (stakingMeta.staking_plan_id !== 'plan_1') {
            return res.status(400).json({
                status: false,
                status_code: 400,
                message: 'This endpoint is only for Plan 1 staking transactions',
                error: {
                    staking_plan_id: stakingMeta.staking_plan_id,
                    required_plan: 'plan_1'
                }
            });
        }
        ...
        if (stakingMeta.staking_capital_withdrawn && stakingMeta.staking_capital_withdrawn.toString().toLowerCase() === 'yes') {
            return res.status(400).json({
                status: false,
                status_code: 400,
                message: 'Capital has already been withdrawn for this Plan 1 staking transaction',
                details: {
        ...
        // Check if capital can be withdrawn (Plan 1 has strict lock duration)
        if (!capitalMetrics.can_withdraw_capital) {
            ...
            return res.status(400).json({
                status: false,
                status_code: 400,
                message: 'Plan 1 Capital lock period not yet completed',
                details: {
                    ...
                    note: 'Plan 1 requires full capital lock duration to be completed before withdrawal'
                }
            });
        }
        ...
        const debitRequestBody = {
            request_id: `plan1_staking_capital_withdraw_request_${request_id}`,
            user_id: user_id,
            amount: staking_amount,
            wallet_id: staking_locked_wallet_id,
            note: 'Plan 1 Staking Capital Withdrawal Request - High-Yield Locked Staking',
            meta_data: {
                staking_transaction_id: stakingTransactionID,
                staking_plan_id: stakingMeta.staking_plan_id,
                staking_plan_name: stakingMeta.staking_plan_name,
                transaction_action_type: 'plan1_staking_capital_withdrawal_debit',
        ...
        const creditRequestBody = {
            request_id: `plan1_staking_capital_withdraw_credit_${request_id}`,
            user_id: user_id,
            amount: staking_amount,
            wallet_id: staking_capital_payment_wallet_id,
            note: 'Plan 1 Staking Capital Withdrawal Credit - High-Yield Locked Staking',
            meta_data: {
                staking_transaction_id: stakingTransactionID,
                staking_alt_transaction_id: debitResponse.data.data.transaction_id,
                staking_plan_id: stakingMeta.staking_plan_id,
                staking_plan_name: stakingMeta.staking_plan_name,
                transaction_action_type: 'plan1_staking_capital_withdrawal_credit',
                transaction_type_category: 'staking',
                transaction_external_processor: 'middleware1',
                transaction_approval_status: 'user_middleware_processed',
                transaction_approval_method: 'middleware'
        ...
        // For Plan 1, keep original ROI end time (no early termination)
        ...
        return res.status(200).json({
            status: true,
            status_code: 200,
            message: 'Plan 1 Staking Capital Withdrawal Completed Successfully',
            data: {
                staking_transaction_id: stakingTransactionID,
                staking_plan_id: stakingMeta.staking_plan_id,
                staking_plan_name: stakingMeta.staking_plan_name,
                ...
                note: 'Plan 1 maintains original ROI end time. ROI continues accumulating until contract end.',
                processed_at: currentTime
            }
        });

    } catch (error) {
        console.error('Plan 1 Capital Withdrawal Error:', error);
        ...
        if (error.response) {
            return res.status(error.response.status).json({
                ...
                message: 'Plan 1 Capital Withdrawal Failed',
                error: {
        ...
        return res.status(500).json({
            status: false,
            status_code: 500,
            message: 'Internal server error during Plan 1 capital withdrawal',
            error: {
