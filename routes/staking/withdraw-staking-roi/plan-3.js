    ...
    try {
        ...
        if (!stakingMetaData) {
            return res.status(404).send({
                status: false,
                status_code: 404,
                message: "Plan 3 staking transaction not found",
                error: { stakingTransactionID }
            });
        }

        // Validate that this is a Plan 3 staking
        if (stakingMetaData.staking_plan_id !== 'plan_3') {
            return res.status(400).send({
                status: false,
                status_code: 400,
        ...
        // Check if ROI can be withdrawn (Plan 3 allows ROI withdrawal even after contract ends)
        ...
        return res.status(200).send({
            status: true,
            status_code: 200,
            message: 'Plan 3 Staking ROI Withdrawal Completed Successfully',
            data: {
                ...
                total_accumulated_roi: stakingMetrics.accumulated_roi_now,
                note: 'Plan 3 allows ROI withdrawal every day with flexible accumulation. ROI stops if capital is withdrawn early.',
                processed_at: currentTime
            }
        });

    } catch (error) {
        console.error('Plan 3 ROI Withdrawal Error:', error);
...
function buildRoiDebitRequestBody(request_id, user_id, amount_to_withdraw, stakingMetaData, stakingMetrics) {
    return {
        request_id: `plan3_staking_roi_withdraw_debit_${request_id}`,
        user_id: user_id,
        amount: amount_to_withdraw,
        wallet_id: stakingMetaData.staking_roi_payment_wallet_id,
        note: `Plan 3 Staking ROI Withdrawal Debit - ${stakingMetaData.staking_plan_name}`,
        meta_data: {
            staking_transaction_id: stakingMetaData.id,
            staking_plan_id: stakingMetaData.staking_plan_id,
            staking_plan_name: stakingMetaData.staking_plan_name,
            transaction_action_type: 'plan3_staking_roi_withdrawal_debit',
...
function buildRoiCreditRequestBody(request_id, user_id, amount_to_withdraw, stakingMetaData, stakingMetrics) {
    // For Plan 3, use pattern-specific withdrawn amount tracking
    ...
    return {
        request_id: `plan3_staking_roi_withdraw_credit_${request_id}`,
        user_id: user_id,
        amount: amount_to_withdraw,
        wallet_id: stakingMetaData.staking_capital_payment_wallet_id,
        note: `Plan 3 Staking ROI Withdrawal Credit - ${stakingMetaData.staking_plan_name}`,
        meta_data: {
            staking_transaction_id: stakingMetaData.id,
            staking_plan_id: stakingMetaData.staking_plan_id,
            staking_plan_name: stakingMetaData.staking_plan_name,
            staking_roi_amount_withdrawn_so_far: parseFloat(withdrawnAmountSoFar) + parseFloat(amount_to_withdraw),
            transaction_action_type: 'plan3_staking_roi_withdrawal_credit',
            transaction_type_category: 'staking',
            transaction_external_processor: 'middleware1',
            transaction_approval_status: 'user_middleware_processed',
            transaction_approval_method: 'middleware'
