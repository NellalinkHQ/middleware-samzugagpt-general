...
/**
 * ${MODULE1_STAKING_PLAN_1_NAME} Staking ROI Withdrawal
 * 
 * Request Body:
...
router.post('/:stakingTransactionID', async function(req, res, next) {
    try {
        ...
        if (stakingMetaData.staking_roi_payment_pattern === "internal_pattern_2") {
            ...
            if (stakingPlanId === 'plan_1' && capitalWithdrawnAt && capitalWithdrawnAt > 0) {
        ...
        console.error(`${MODULE1_STAKING_PLAN_1_NAME} ROI Withdrawal Error:`, error);
        return handleTryCatchError(res, error);
    }
});

// POST /staking/withdraw-staking-roi/plan-1/blockchain-external/:stakingTransactionID
router.post('/blockchain-external/:stakingTransactionID', async (req, res) => {
    try {
        ...
        console.error(`${MODULE1_STAKING_PLAN_1_NAME} ROI External Withdrawal Error:`, error);
        return handleTryCatchError(res, error);
    }
});
