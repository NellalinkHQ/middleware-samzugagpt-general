...
router.get('/', async function(req, res, next) {
    try {
        ...
        function convertNumericStrToFloat(obj) {
            for (const key in obj) {
                if (!isNaN(obj[key]) && !Array.isArray(obj[key])) {
                    obj[key] = parseFloat(obj[key]);
                } else if (typeof obj[key] === 'object') {
                    convertNumericStrToFloat(obj[key]);
                }
            }
        }
        ...
        if (getMetaResponse.data.data) {
            ...
            let swap_minimum_quantity_per_request = parseFloat(getMetaResponse.data.data[`swap_minimum_quantity_per_request_${swap_rate_key}`]) || 0;
            let swap_maximum_quantity_per_request = parseFloat(getMetaResponse.data.data[`swap_maximum_quantity_per_request_${swap_rate_key}`]) || 0;
            ...
            convertNumericStrToFloat(conversion_rate_data);
